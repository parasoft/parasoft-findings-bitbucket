...
export interface RunOptions {
    ...
    /* Specify a quality gate for a Bitbucket build. The value is in the format: '{BITBUCKET_SECURITY_LEVEL=THRESHOLD}', eg. {'ALL=5', 'HIGH=2'} */
...
export class StaticAnalysisParserRunner {
    ...
    private async convertReportWithJava(javaPath: string, sourcePath: string): Promise<string> {
        ...
        logger.debug(`${javaPath} ${args.join(' ')}`);
